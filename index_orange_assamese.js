...
// d3.csv("./result.csv", function(data) {
  ...
  var dataOrganized = d3.nest() // organize all data by country 
    .key(function(d) { if(d.color == 'orange'){ return d.state_assamese;}})  // keys are country, values are the data associated w the country
    .entries(data);
    //console.log(data);
  ...
  svg.each(function(d){
    ...
    svg.append("rect")
      ...
      .on("mousemove", function(){
        ...
        focus.select(".tooltip-info-avg").text("সাদিনৰ গড়:  "+ d3.format(",")(Math.floor(+d.recent_new)));
        focus.select(".tooltip-info-new").text("নতুন:  "+ d3.format(",")(+d.new_cases));
      });
  })
  ...
  svg
    ...
    .text(function(d){ return("পজিটিভৰ সংখ্যা: "+d3.format(",")(d.values[0].total_cases))})
    .style("fill", "grey");
    ...
    .text(function(d){ return("শেহতীয়া সাদিনৰ গড়: "+ d3.format(",")(Math.floor(d.values[d.values.length-1].recent_new)))})
    .style("fill", "grey");
})